...
const VisioPage = () => {
  ...
  useEffect(() => {
    const fetchEntretienId = async () => {
      ...
      try {
        const { data } = await axios.get(`http://localhost:3004/api/entretiens/by-jitsi-url`, { params: { url: jitsiUrlParam } });
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        ...
        {error === 'MEETING_ENDED' && (
          <div className="flex flex-col items-center justify-center my-10">
            ...
            <div className="text-gray-500 text-base text-center">Cet entretien est terminé.<br/>Merci d'avoir utilisé notre plateforme vidéo.<br/>Vous pouvez fermer cette page ou retourner à votre tableau de bord.</div>
            <a href="/Candidat/dashboard" className="mt-6 px-6 py-2 rounded-lg bg-blue-600 text-white font-bold shadow hover:bg-blue-700 transition">Retour au tableau de bord</a>
          </div>
        )}
        {error === 'MEETING_CANCELED' && (
          <div className="flex flex-col items-center justify-center my-10">
            <svg className="w-16 h-16 text-red-400 mb-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-3-3v6" /></svg>
            <div className="text-red-700 text-lg font-bold mb-2">L'entretien a été annulé</div>
            <div className="text-gray-500 text-base text-center">Cet entretien a été annulé.<br/>Vous pouvez fermer cette page ou retourner à votre tableau de bord.</div>
            <a href="/Candidat/dashboard" className="mt-6 px-6 py-2 rounded-lg bg-blue-600 text-white font-bold shadow hover:bg-blue-700 transition">Retour au tableau de bord</a>
          </div>
        )}
        {error && error !== 'MEETING_ENDED' && error !== 'MEETING_CANCELED' && (
          <div className="flex flex-col items-center justify-center my-10">
            ...
            <div className="text-gray-400 text-base">Merci d'utiliser notre plateforme d'entretien vidéo.<br/>Vous pouvez fermer cette page ou retourner à votre tableau de bord.</div>
            <a href="/Candidat/dashboard" className="mt-6 px-6 py-2 rounded-lg bg-blue-600 text-white font-bold shadow hover:bg-blue-700 transition">Retour au tableau de bord</a>
          </div>
        )}
